...
const agesList = async (result) => {
    await connection.query('SELECT a.nit, e.nombre, e.cod_reg, r.nom_reg, COALESCE(SUM(CASE WHEN DATEDIFF(DATE(NOW()), fecha_envio) < 31 then valor_factura end),0) edad1, COALESCE(SUM(CASE WHEN DATEDIFF(DATE(NOW()), fecha_envio) > 30 AND DATEDIFF(DATE(NOW()), fecha_envio) < 61 then valor_factura end),0) edad2, COALESCE(SUM(CASE WHEN DATEDIFF(DATE(NOW()), fecha_envio) > 60 AND DATEDIFF(DATE(NOW()), fecha_envio) < 91 THEN valor_factura END),0) edad3, COALESCE(SUM(CASE WHEN DATEDIFF(DATE(NOW()), fecha_envio) > 90 AND DATEDIFF(DATE(NOW()), fecha_envio) < 181 THEN valor_factura END),0) edad3, COALESCE(SUM(CASE WHEN DATEDIFF(DATE(NOW()), fecha_envio) > 180 AND DATEDIFF(DATE(NOW()), fecha_envio) < 361 THEN valor_factura END),0) edad4, COALESCE(SUM(CASE WHEN DATEDIFF(DATE(NOW()), fecha_envio) > 360 then valor_factura end),0) edad5 FROM consolidado as a INNER JOIN empresa AS e ON a.nit = e.nit INNER JOIN regimen as r ON r.cod_reg = e.cod_reg GROUP BY a.nit order by r.nom_reg', (error, ages) => {
        if(error){
            return result(error)
        }else{
            return result(ages)
